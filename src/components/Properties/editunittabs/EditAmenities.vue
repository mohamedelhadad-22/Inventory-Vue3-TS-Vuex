...
export default {
  ...
  setup(props, { emit }) {
    const amenities = ref<UnitDataTableItem>({ ...props.editingUnitData })
    console.log('Amenities', amenities.value.amenities)

    // Watch for changes in editingUnitData and update amenities accordingly
    watch(
      () => props.editingUnitData,
      (newValue) => {
        // Assuming your UnitDataTableItem has corresponding properties for amenities
        amenities.value = {
          ...amenities.value,
          ...newValue
        }
        emit('update:amenities', amenities.value)
      },
      { immediate: true }
    )

    // Watch for changes in amenities and emit update event
    watch(
      amenities,
      (newValue) => {
    ...
    // Return the reactive amenities ref for use in the template
    return {
      amenities
    }
  }
